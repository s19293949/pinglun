    ...
    def __init__(self,config):
        ...
        with tf.variable_scope("Net",initializer=tf.orthogonal_initializer()):
            ...
            with tf.name_scope("output"):
                ...
                self.predictions=tf.argmax(self.result,1,name="predictions")  #输出最大值的下标
        ...
        #计算损失
            with tf.name_scope("loss"):
                losses=tf.nn.softmax_cross_entropy_with_logits_v2(logits=self.result, labels=self.input_y)
            ...
            with tf.name_scope("train_op"):
                ...
                optimizer = tf.train.AdamOptimizer(self.l2_rate)
                optimizer.apply_gradients(zip(grads, tvars))   #梯度修剪，防止梯度消失或梯度爆炸
